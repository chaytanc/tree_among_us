...
class TestData(unittest.TestCase):
    ...
    def test_raw_data_size(self):
        ...
        self.assertEqual(pred_len, len(self.data.averaged_brain_states))


    def test_same_size_samples(self):
        for pair in self.data.brain_states:
            pass
    ...
    def test_averaged_data(self):
        ...
        self.assertTrue(torch.all(torch.eq(first_sample_supposed_averages, self.data.averaged_brain_states[0][0])))
        self.assertTrue(torch.all(torch.eq(second_sample_supposed_averages, self.data.averaged_brain_states[1][0])))
        self.assertTrue(torch.all(torch.eq(first_sample_supposed_averages, self.data.averaged_brain_states[2][0])))
        self.assertTrue(torch.all(torch.eq(second_sample_supposed_averages, self.data.averaged_brain_states[3][0])))
